...
export interface ISerpYahooOrganicLiveRegularRequestInfo   {
        ...
        /** parsing depth optional field number of results in SERP default value: 6 max value: 200 Your account will be billed per each SERP; Each Yahoo SERP can contain fewer than 10 results, so setting depth above the default value may result in additional charges ; The cost can be calculated on the Pricing page. */
        ...
        /** target domain, subdomain, or wildcard value required field if stop_crawl_on_match is specified; required field if stop_crawl_on_match is specified specify a target domain, subdomain, or wildcard value; Note: domain or subdomain must be specified without a request protocol; example: 'match_value': 'dataforseo.com', 'match_value': '/blog/post-*' */
