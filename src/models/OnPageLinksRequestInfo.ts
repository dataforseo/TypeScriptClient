export interface IOnPageLinksRequestInfo   {
        ...
        /** *offset in the results array of returned links* optional field default value: `0` maximum value: `2000000` if you specify the `10` value, the first ten links in the results array will be omitted and the data will be provided for the successive links */
        offset?: number | undefined
        
        /** *array of results filtering parameters* optional field **you can add several filters at once (8 filters maximum)** you should set a logical operator `and`, `or` between the conditions the following operators are supported: `regex`, `not_regex`, `=`, `<>`, `in`, `not_in`, `like`, `not_like` you can use the `%` operator with `like` and `not_like` to match any string of zero or more characters example: `['direction','=','external']` `[['domain_to','<>','example.com'],'and',['link_from','not_like','%example.com/blog%']]` `[['direction','=','external'],'and',[['link_from','like','%example.com/blog%'],'or',['link_from','like','%example.com/help%']]]` The full list of possible filters is available [by this link.](/v3/on_page/filters_and_thresholds/?bash) */
