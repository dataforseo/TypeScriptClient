export interface IDataforseoLabsGoogleRankedKeywordsLiveRequestInfo   {
        ...
        /** *array of results filtering parameters* optional field **you can add several filters at once (8 filters maximum)** you should set a logical operator `and`, `or` between the conditions the following operators are supported: `regex`, `not_regex`, ``, `>=`, `=`, `<>`, `in`, `not_in`, `match`, `not_match`, `ilike`, `not_ilike`, `like`, `not_like` you can use the `%` operator with `like` and `not_like`, as well as `ilike` and `not_ilike` to match any string of zero or more characters example: `['ranked_serp_element.serp_item.rank_group','','paid']]` `[['keyword_data.keyword_info.search_volume','<>',0],'and',[['ranked_serp_element.serp_item.type','<>','paid'],'or',['ranked_serp_element.serp_item.is_malicious','=',false]]]` if you want to get the keywords a particular webpage ranks for, you can use a `[target](/v3/dataforseo_labs/google/ranked_keywords/live/#target)` field or filter by the `ranked_serp_element.serp_item.relative_url` parameter example: `['ranked_serp_element.serp_item.relative_url', '=', '/apis/rank-tracker-api']` for more information about filters, please refer to [Dataforseo Labs - Filters](/v3/dataforseo_labs/filters) or this [help center guide](https://dataforseo.com/help-center/how-to-use-filters-in-dataforseo-labs-api) */
