...
export interface IKeywordsDataBingKeywordsForSiteTaskGetResultInfo   {
        ...
        /** *cost-per-click* represents the average cost per click (USD) historically paid for the keyword. if there is no data the value is `null` */
        cpc?: number | undefined
        
        /** *monthly average search volume rate*represents the (approximate) number of searches for the given keyword idea on Bing search engine depending on the user’s targeting if there is no data then the value is_`null`n */
        search_volume?: number | undefined
        
        /** *product and service categories* legacy field, the value will always be `null` */
        categories?: string[] | undefined
        
        /** *monthly searches* represents the (approximate) number of searches on this keyword idea (as available for the past twelve months), targeted to the specified geographic locations search volume is rounded to the closest decimal values if there is no data the value is `null` */
