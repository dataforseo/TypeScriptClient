...
**check_url** | **string** | direct URL to search engine resultsyou can use it to make sure that we provided exact results |[optional]|
...
**search_results** | **ChatgptSearchResult[]** | array of search resultsall web search outputs the model retrieved when looking up information, including duplicates and unused entries |[optional]|
**sources** | **SourceInfo[]** | array of sourcesthe sources the model actually cited or relied on in its final answer |[optional]|
...
**brand_entities** | **ChatGptBrandEntity[]** | array of brand entitiescontains information on brands mentioned in the response |[optional]|
**se_results_count** | **number** | total number of results |[optional]|
**item_types** | **string[]** | types of search resultscontains types of search results (items) found in SERP.possible item types:chat_gpt_text, chat_gpt_table, chat_gpt_navigation_list, chat_gpt_images, chat_gpt_local_businesses, chat_gpt_products |[optional]|
