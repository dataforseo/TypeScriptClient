...
**spell** | **SpellInfo** | *autocorrection of the search engine*. if the search engine provided results for a keyword that was corrected, we will specify the keyword corrected by the search engine and the type of autocorrection;. if there is none, the value is `null` |[optional]|
**refinement_chips** | **RefinementChipsInfo** | *search refinement chips*. if there are none, the value is `null` |[optional]|
