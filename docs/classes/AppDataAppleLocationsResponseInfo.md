[dataforseo-client](../README.md) / [Exports](../modules.md) / AppDataAppleLocationsResponseInfo

# Class: AppDataAppleLocationsResponseInfo

## Hierarchy

- [`BaseResponseInfo`](BaseResponseInfo.md)

  ↳ **`AppDataAppleLocationsResponseInfo`**

## Implements

- [`IAppDataAppleLocationsResponseInfo`](../interfaces/IAppDataAppleLocationsResponseInfo.md)

## Indexable

▪ [key: `string`]: `any`

## Table of contents

### Constructors

- [constructor](AppDataAppleLocationsResponseInfo.md#constructor)

### Properties

- [cost](AppDataAppleLocationsResponseInfo.md#cost)
- [status\_code](AppDataAppleLocationsResponseInfo.md#status_code)
- [status\_message](AppDataAppleLocationsResponseInfo.md#status_message)
- [tasks](AppDataAppleLocationsResponseInfo.md#tasks)
- [tasks\_count](AppDataAppleLocationsResponseInfo.md#tasks_count)
- [tasks\_error](AppDataAppleLocationsResponseInfo.md#tasks_error)
- [time](AppDataAppleLocationsResponseInfo.md#time)
- [version](AppDataAppleLocationsResponseInfo.md#version)

### Methods

- [init](AppDataAppleLocationsResponseInfo.md#init)
- [toJSON](AppDataAppleLocationsResponseInfo.md#tojson)
- [fromJS](AppDataAppleLocationsResponseInfo.md#fromjs)

## Constructors

### constructor

• **new AppDataAppleLocationsResponseInfo**(`data?`): [`AppDataAppleLocationsResponseInfo`](AppDataAppleLocationsResponseInfo.md)

#### Parameters

| Name | Type |
| :------ | :------ |
| `data?` | [`IAppDataAppleLocationsResponseInfo`](../interfaces/IAppDataAppleLocationsResponseInfo.md) |

#### Returns

[`AppDataAppleLocationsResponseInfo`](AppDataAppleLocationsResponseInfo.md)

#### Overrides

[BaseResponseInfo](BaseResponseInfo.md).[constructor](BaseResponseInfo.md#constructor)

#### Defined in

[main.ts:182879](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L182879)

## Properties

### cost

• `Optional` **cost**: `number`

total tasks cost, USD

#### Implementation of

[IAppDataAppleLocationsResponseInfo](../interfaces/IAppDataAppleLocationsResponseInfo.md).[cost](../interfaces/IAppDataAppleLocationsResponseInfo.md#cost)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[cost](BaseResponseInfo.md#cost)

#### Defined in

[main.ts:22510](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22510)

___

### status\_code

• `Optional` **status\_code**: `number`

general status code
you can find the full list of the response codes here

#### Implementation of

[IAppDataAppleLocationsResponseInfo](../interfaces/IAppDataAppleLocationsResponseInfo.md).[status_code](../interfaces/IAppDataAppleLocationsResponseInfo.md#status_code)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[status_code](BaseResponseInfo.md#status_code)

#### Defined in

[main.ts:22503](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22503)

___

### status\_message

• `Optional` **status\_message**: `string`

general informational message
you can find the full list of general informational messages here

#### Implementation of

[IAppDataAppleLocationsResponseInfo](../interfaces/IAppDataAppleLocationsResponseInfo.md).[status_message](../interfaces/IAppDataAppleLocationsResponseInfo.md#status_message)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[status_message](BaseResponseInfo.md#status_message)

#### Defined in

[main.ts:22506](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22506)

___

### tasks

• `Optional` **tasks**: [`AppDataAppleLocationsTaskInfo`](AppDataAppleLocationsTaskInfo.md)[]

array of tasks

#### Implementation of

[IAppDataAppleLocationsResponseInfo](../interfaces/IAppDataAppleLocationsResponseInfo.md).[tasks](../interfaces/IAppDataAppleLocationsResponseInfo.md#tasks)

#### Defined in

[main.ts:182875](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L182875)

___

### tasks\_count

• `Optional` **tasks\_count**: `number`

the number of tasks in the tasks array

#### Implementation of

[IAppDataAppleLocationsResponseInfo](../interfaces/IAppDataAppleLocationsResponseInfo.md).[tasks_count](../interfaces/IAppDataAppleLocationsResponseInfo.md#tasks_count)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[tasks_count](BaseResponseInfo.md#tasks_count)

#### Defined in

[main.ts:22512](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22512)

___

### tasks\_error

• `Optional` **tasks\_error**: `number`

the number of tasks in the tasks array returned with an error

#### Implementation of

[IAppDataAppleLocationsResponseInfo](../interfaces/IAppDataAppleLocationsResponseInfo.md).[tasks_error](../interfaces/IAppDataAppleLocationsResponseInfo.md#tasks_error)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[tasks_error](BaseResponseInfo.md#tasks_error)

#### Defined in

[main.ts:22514](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22514)

___

### time

• `Optional` **time**: `string`

total execution time, seconds

#### Implementation of

[IAppDataAppleLocationsResponseInfo](../interfaces/IAppDataAppleLocationsResponseInfo.md).[time](../interfaces/IAppDataAppleLocationsResponseInfo.md#time)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[time](BaseResponseInfo.md#time)

#### Defined in

[main.ts:22508](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22508)

___

### version

• `Optional` **version**: `string`

the current version of the API

#### Implementation of

[IAppDataAppleLocationsResponseInfo](../interfaces/IAppDataAppleLocationsResponseInfo.md).[version](../interfaces/IAppDataAppleLocationsResponseInfo.md#version)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[version](BaseResponseInfo.md#version)

#### Defined in

[main.ts:22500](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22500)

## Methods

### init

▸ **init**(`_data?`): `void`

#### Parameters

| Name | Type |
| :------ | :------ |
| `_data?` | `any` |

#### Returns

`void`

#### Overrides

[BaseResponseInfo](BaseResponseInfo.md).[init](BaseResponseInfo.md#init)

#### Defined in

[main.ts:182883](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L182883)

___

### toJSON

▸ **toJSON**(`data?`): `any`

#### Parameters

| Name | Type |
| :------ | :------ |
| `data?` | `any` |

#### Returns

`any`

#### Overrides

[BaseResponseInfo](BaseResponseInfo.md).[toJSON](BaseResponseInfo.md#tojson)

#### Defined in

[main.ts:182905](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L182905)

___

### fromJS

▸ **fromJS**(`data`): [`AppDataAppleLocationsResponseInfo`](AppDataAppleLocationsResponseInfo.md)

#### Parameters

| Name | Type |
| :------ | :------ |
| `data` | `any` |

#### Returns

[`AppDataAppleLocationsResponseInfo`](AppDataAppleLocationsResponseInfo.md)

#### Overrides

[BaseResponseInfo](BaseResponseInfo.md).[fromJS](BaseResponseInfo.md#fromjs)

#### Defined in

[main.ts:182898](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L182898)
