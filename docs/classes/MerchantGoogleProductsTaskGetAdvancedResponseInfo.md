[dataforseo-client](../README.md) / [Exports](../modules.md) / MerchantGoogleProductsTaskGetAdvancedResponseInfo

# Class: MerchantGoogleProductsTaskGetAdvancedResponseInfo

## Hierarchy

- [`BaseResponseInfo`](BaseResponseInfo.md)

  ↳ **`MerchantGoogleProductsTaskGetAdvancedResponseInfo`**

## Implements

- [`IMerchantGoogleProductsTaskGetAdvancedResponseInfo`](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md)

## Indexable

▪ [key: `string`]: `any`

## Table of contents

### Constructors

- [constructor](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#constructor)

### Properties

- [cost](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#cost)
- [status\_code](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#status_code)
- [status\_message](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#status_message)
- [tasks](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#tasks)
- [tasks\_count](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#tasks_count)
- [tasks\_error](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#tasks_error)
- [time](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#time)
- [version](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#version)

### Methods

- [init](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#init)
- [toJSON](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#tojson)
- [fromJS](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md#fromjs)

## Constructors

### constructor

• **new MerchantGoogleProductsTaskGetAdvancedResponseInfo**(`data?`): [`MerchantGoogleProductsTaskGetAdvancedResponseInfo`](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md)

#### Parameters

| Name | Type |
| :------ | :------ |
| `data?` | [`IMerchantGoogleProductsTaskGetAdvancedResponseInfo`](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md) |

#### Returns

[`MerchantGoogleProductsTaskGetAdvancedResponseInfo`](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md)

#### Overrides

[BaseResponseInfo](BaseResponseInfo.md).[constructor](BaseResponseInfo.md#constructor)

#### Defined in

[main.ts:165578](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L165578)

## Properties

### cost

• `Optional` **cost**: `number`

total tasks cost, USD

#### Implementation of

[IMerchantGoogleProductsTaskGetAdvancedResponseInfo](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md).[cost](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md#cost)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[cost](BaseResponseInfo.md#cost)

#### Defined in

[main.ts:22510](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22510)

___

### status\_code

• `Optional` **status\_code**: `number`

general status code
you can find the full list of the response codes here

#### Implementation of

[IMerchantGoogleProductsTaskGetAdvancedResponseInfo](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md).[status_code](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md#status_code)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[status_code](BaseResponseInfo.md#status_code)

#### Defined in

[main.ts:22503](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22503)

___

### status\_message

• `Optional` **status\_message**: `string`

general informational message
you can find the full list of general informational messages here

#### Implementation of

[IMerchantGoogleProductsTaskGetAdvancedResponseInfo](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md).[status_message](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md#status_message)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[status_message](BaseResponseInfo.md#status_message)

#### Defined in

[main.ts:22506](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22506)

___

### tasks

• `Optional` **tasks**: [`MerchantGoogleProductsTaskGetAdvancedTaskInfo`](MerchantGoogleProductsTaskGetAdvancedTaskInfo.md)[]

array of tasks

#### Implementation of

[IMerchantGoogleProductsTaskGetAdvancedResponseInfo](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md).[tasks](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md#tasks)

#### Defined in

[main.ts:165574](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L165574)

___

### tasks\_count

• `Optional` **tasks\_count**: `number`

the number of tasks in the tasks array

#### Implementation of

[IMerchantGoogleProductsTaskGetAdvancedResponseInfo](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md).[tasks_count](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md#tasks_count)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[tasks_count](BaseResponseInfo.md#tasks_count)

#### Defined in

[main.ts:22512](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22512)

___

### tasks\_error

• `Optional` **tasks\_error**: `number`

the number of tasks in the tasks array returned with an error

#### Implementation of

[IMerchantGoogleProductsTaskGetAdvancedResponseInfo](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md).[tasks_error](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md#tasks_error)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[tasks_error](BaseResponseInfo.md#tasks_error)

#### Defined in

[main.ts:22514](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22514)

___

### time

• `Optional` **time**: `string`

total execution time, seconds

#### Implementation of

[IMerchantGoogleProductsTaskGetAdvancedResponseInfo](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md).[time](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md#time)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[time](BaseResponseInfo.md#time)

#### Defined in

[main.ts:22508](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22508)

___

### version

• `Optional` **version**: `string`

the current version of the API

#### Implementation of

[IMerchantGoogleProductsTaskGetAdvancedResponseInfo](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md).[version](../interfaces/IMerchantGoogleProductsTaskGetAdvancedResponseInfo.md#version)

#### Inherited from

[BaseResponseInfo](BaseResponseInfo.md).[version](BaseResponseInfo.md#version)

#### Defined in

[main.ts:22500](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L22500)

## Methods

### init

▸ **init**(`_data?`): `void`

#### Parameters

| Name | Type |
| :------ | :------ |
| `_data?` | `any` |

#### Returns

`void`

#### Overrides

[BaseResponseInfo](BaseResponseInfo.md).[init](BaseResponseInfo.md#init)

#### Defined in

[main.ts:165582](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L165582)

___

### toJSON

▸ **toJSON**(`data?`): `any`

#### Parameters

| Name | Type |
| :------ | :------ |
| `data?` | `any` |

#### Returns

`any`

#### Overrides

[BaseResponseInfo](BaseResponseInfo.md).[toJSON](BaseResponseInfo.md#tojson)

#### Defined in

[main.ts:165604](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L165604)

___

### fromJS

▸ **fromJS**(`data`): [`MerchantGoogleProductsTaskGetAdvancedResponseInfo`](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md)

#### Parameters

| Name | Type |
| :------ | :------ |
| `data` | `any` |

#### Returns

[`MerchantGoogleProductsTaskGetAdvancedResponseInfo`](MerchantGoogleProductsTaskGetAdvancedResponseInfo.md)

#### Overrides

[BaseResponseInfo](BaseResponseInfo.md).[fromJS](BaseResponseInfo.md#fromjs)

#### Defined in

[main.ts:165597](https://github.com/dataforseo/TypeScriptClient/blob/7ca1aa4/main.ts#L165597)
