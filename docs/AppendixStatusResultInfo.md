...
**status** | **string** | *current status*. you can find all information about the statuses of our endpoints for the last 60 days [here](https://status.dataforseo.com/). the list of possible current statuses:. `major_outage`. `partial_outage`. `long_response_time`. `long_execution_time`. `webhook_delay`. `send_delay` |[optional]|
