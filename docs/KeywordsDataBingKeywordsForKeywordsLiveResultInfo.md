...
**search_volume** | **number** | *monthly average search volume rate*. represents the (approximate) number of searches for the keyword on the Bing search engine, depending on the user’s targetingsearch volume is rounded to the closest decimal values. if there is no data, then the value is_`null`n |[optional]|
