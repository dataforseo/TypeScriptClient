# LocationInfo

## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
**name** | **string** | *name of the event's venue* |[optional]|
**address** | **string** | *address of the event's venue* |[optional]|
**url** | **string** | *search URL with refinement parameters* |[optional]|
**cid** | **string** | *google-defined client id*. can be used with [Google Reviews API](/v3/business_data/google/reviews/task_post/?php) to get a full list of reviews |[optional]|
**feature_id** | **string** | *the unique identifier of the element in SERP* |[optional]|