...
**search_volume** | **number** | monthly average search volume rate. represents the (approximate) number of searches for the given keyword idea on Bing search engine depending on the user’s targeting. if there is no data then the value is null |[optional]|
