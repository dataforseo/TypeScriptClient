...
**spell** | **SpellInfo** | *autocorrection of the search engine*. if the search engine provided results for a keyword that was corrected, we will specify the keyword corrected by the search engine and the type of autocorrection;. in this case, will equal `null` |[optional]|
**refinement_chips** | **RefinementChipsInfo** | *search refinement chips* |[optional]|
**item_types** | **string[]** | *types of search results in SERP*. contains types of search results (`items`) found in SERP.. possible item types:. `autocomplete` |[optional]|
